...
class group_svr_controller():
    ...
    def __init__(self, path):
        ...
        self.tap_list = np.array((self.total_svrs,self.total_timesteps),dtype=int)  # Store a 2D array that will hold all of our tap changes for each SVR
    ...
    def loadshape_to_tap(self): #Simple way, a more advanced method can be developed later
        ...
        for svr in range(self.total_svrs):
            for each in range(multipliers):
                ...
                self.tap_list[svr][each] = multipliers[each] # Assign the taps. All SVRs will run off the same tap for now.
